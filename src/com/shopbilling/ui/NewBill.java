...
public class NewBill extends JInternalFrame {
	...
	public NewBill() {
		...
		billDetailsPanel.setBounds(10, 11, 806, 131);
		itemDetailsPanel = new JPanel();
		itemDetailsPanel.setBounds(10, 148, 806, 450);
		...
		layout.row().grid(new JLabel("Customer Pending Amt :")).addMulti(icon,customerExistingBalance).empty().add(addCustomer);
		...
		paymentDetails.setBounds(826, 148, 330, 450);
		...
		panelBarCode.setBounds(826, 73, 330, 64);
		...
		amount.setBounds(688, 27, 102, 27);
		...
		btn_refresh.setBounds(1157, 15, 24, 24);
		...
		btnSaveBill.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(customerMobileNo.getText().equals("") || customerName.getText().equals("")
					...
					JOptionPane.showMessageDialog(getContentPane(), "Please Enter Select/Customer Details !");
					...
					if(addCustomer.isEnabled()){
						...
						double custBal = Double.valueOf(customerExistingBalance.getText());
						if("PENDING".equals((String)cb_PaymentMode.getSelectedItem()) && custBal>=0){
							prepareBill();
							...
							if(status.getStatusCode() ==0 && statusAddPendingAmt.getStatusCode() ==0 
									...
									ProductHistoryServices.addProductStockLedger(getProductList(itemList), AppConstants.STOCK_OUT, AppConstants.SALES);
							...
							if("PENDING".equals((String)cb_PaymentMode.getSelectedItem())){
								JOptionPane.showMessageDialog(getContentPane(), "Customer Balance is Negative. Please check customer payment history !");
							}
						}
						//Cash Bill
						if(!"PENDING".equals((String)cb_PaymentMode.getSelectedItem())){
		...
		btnCashHelp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				CashHelp cash = new CashHelp(mainFrame,tf_NetSalesAmt.getText());
	...
	private List<Product> getProductList(List<ItemDetails> itemList){
		...
		for(ItemDetails item : itemList){
			...
			p.setDescription("Sales Based on Bill No.: "+item.getBillNumber());
			productList.add(p);
		}
		return productList;

...
public class SalesReportExportUI extends JInternalFrame {
	...
	private void fillReportTable(){
		List<BillDetails> billList= ProductServices.getBillDetails(fromDateChooser.getDate()==null?null:new java.sql.Date(fromDateChooser.getDate().getTime()),toDateChooser.getDate()==null?null:new java.sql.Date(toDateChooser.getDate().getTime()),null);
		calculateConsolidateValues(billList);
		if(billList.isEmpty()){
			JOptionPane.showMessageDialog(getContentPane(), "No Records found for the given period !");
		}else{
			List<Map<String,?>> dataSourceMap = JasperServices.createDateForSalesReport(billList,fromDateChooser.getDate()!=null?PDFUtils.getFormattedDate(fromDateChooser.getDate()):"___",toDateChooser.getDate()!=null?PDFUtils.getFormattedDate(toDateChooser.getDate()):"___",pendingPayment,cashPayment,totalAmount,totalQty,totalNoOfItems);
			boolean isSucess=JasperUtils.createPDF(dataSourceMap,AppConstants.SALES_REPORT_JASPER,"Sales_Report");
			if(isSucess){

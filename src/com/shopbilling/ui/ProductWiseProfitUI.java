...
public class ProductWiseProfitUI extends JInternalFrame {
	...
	static{
		customerMap = CustomerHistoryServices.getCustomerMap();
		}  
	/**
	 * Create the frame.
	 */
	public ProductWiseProfitUI() {
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(200, 85, 1107, 698);
		setTitle("Product Wise Profit");
		...
		reportModel = new DefaultTableModel(){
		 ...
		 reportModel.setColumnIdentifiers(new String[] {
				 "Product Code", "Product Name","Total Qty","Product MRP","Total MRP Amt","Profit Amount"}
	       );
		...
		table.getColumnModel().getColumn(1).setPreferredWidth(290);
		 ...
		 panel_2.setBorder(new TitledBorder(null, "Top Product", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		 ...
		 lblCustomerNameValue.setBounds(535, 40, 339, 31);
		 ...
		 lblCustomerName.setBounds(535, 11, 339, 31);
	...
	private void fillReportTable(){
		List<ProductAnalysis> list= ProductSaleAnalysisServices.getProductWiseProfit(fromDateChooser.getDate()==null?null:new java.sql.Date(fromDateChooser.getDate().getTime()),toDateChooser.getDate()==null?null:new java.sql.Date(toDateChooser.getDate().getTime()));
		...
		if(list.isEmpty()){
			...
			for(ProductAnalysis b : list){
				...
				reportModel.addRow(new Object[]{b.getProductCode(),b.getProductName(),b.getTotalQty(),PDFUtils.getDecimalFormat(b.getProductMRP()),PDFUtils.getDecimalFormat(b.getTotalMRPAmount()),PDFUtils.getDecimalFormat(b.getProfit())});
			}
			setReportValues();
		}
	}

...
public class SaleReport extends JInternalFrame {
	...
	private static double pendingPayment=0;
	...
	public SaleReport() {
		...
		lblToDate.setBounds(284, 19, 79, 28);
		panel.add(lblToDate);
		...
		btnGetReport.setBounds(923, 15, 127, 34);
		...
		toDateChooser.setBounds(373, 19, 133, 28);
		toDateChooser.setDate(new Date());
		panel.add(toDateChooser);
		
		JLabel label = new JLabel("Customer Mobile :");
		label.setHorizontalAlignment(SwingConstants.RIGHT);
		label.setFont(new Font("Tahoma", Font.BOLD, 14));
		label.setBounds(536, 19, 146, 28);
		panel.add(label);
		...
		scrollPane.setViewportView(table);
		 ...
		 Action viewBillAction = new AbstractAction()
		 {
		     public void actionPerformed(ActionEvent e)
		     {
        		...
        		if(billsMap.containsKey(billNo)){
        			 ViewCustomerBill vb = new ViewCustomerBill(billsMap.get(billNo),mainFrame);
    		    	 vb.setVisible(true);
        		}
		 ...
		 panel_2.setBounds(50, 449, 1150, 131);
